...
var userAgent = !isNode && navigator && navigator.userAgent && navigator.userAgent.toLowerCase();
var isElectron = userAgent && userAgent.indexOf(' electron/') > -1;

var isMobile = !isNode && function () {
  if (isElectron) {
    return false;
  }
  var check = false;
...
function gunAsAnotherUser(gun, key, f) {
  ...
  var gun2 = new Gun({ radisk: false, peers: Object.keys(gun._.opt.peers) }); // TODO: copy other options too
  var user = gun2.user();
  user.auth(key);
  setTimeout(function () {
...
async function loadGunDepth(chain) {
  ...
  return chain.then().then(function (layer) {
    ...
    var bcount = 0;
    var promises = Object.keys(layer).map(function (key) {
