...
var Channel = function () {
  ...
  */
  ...
  Channel.prototype.send = async function send(msg) {
    ...
    if (this.uuid) {
      var encrypted = await Gun.SEA.encrypt(JSON.stringify(msg), this.getMyGroupSecret());
      var mySecretUuid = await this.getMySecretUuid();
      ...
      for (var i = 0; i < keys.length; i++) {
        var _encrypted = await Gun.SEA.encrypt(JSON.stringify(msg), (await this.getSecret(keys[i])));
  ...
  */
  ...
  Channel.prototype.putGroup = async function putGroup(key, value) {
    ...
    var encrypted = await Gun.SEA.encrypt(JSON.stringify(value), this.getMyGroupSecret());
    var mySecretUuid = await this.getMySecretUuid();
  ...
  Channel.prototype.putDirect = async function putDirect(key, value) {
    ...
    var keys = this.getCurrentParticipants();
    for (var i = 0; i < keys.length; i++) {
      var encrypted = await Gun.SEA.encrypt(JSON.stringify(value), (await this.getSecret(keys[i])));
      var ourSecretChannelId = await this.getOurSecretChannelId(keys[i]);
