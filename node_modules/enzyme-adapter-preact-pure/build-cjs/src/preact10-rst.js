...
var shallow_render_utils_js_1 = require("./shallow-render-utils.js");
...
function rstNodeFromVNode(node) {
    ...
    if (node.type === preact_1.Fragment) {
        return rstNodesFromChildren(preact10_internals_js_1.getChildren(node));
    }
    var component = preact10_internals_js_1.getComponent(node);
    if (component) {
        return rstNodeFromComponent(node, component);
    }
    if (!preact10_internals_js_1.getDOMNode(node)) {
    ...
    return {
        ...
        key: node.key || null,
        ref: node.ref || null,
        instance: preact10_internals_js_1.getDOMNode(node),
        rendered: rstNodesFromChildren(preact10_internals_js_1.getChildren(node)),
    };
}
...
function rstNodeFromElement(node) {
    ...
    var children = compat_js_1.childElements(node).map(rstNodeFromElement);
...
function rstNodeFromComponent(vnode, component) {
    ...
    var rendered = rstNodesFromChildren(preact10_internals_js_1.getLastRenderOutput(component));
    // If this was a shallow-rendered component, set the RST node's type to the
    // real component function/class.
    var shallowRenderedType = shallow_render_utils_js_1.getRealType(component);
...
function getNode(container) {
    var rendered = preact10_internals_js_1.getLastVNodeRenderedIntoContainer(container);
